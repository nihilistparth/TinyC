...
*/
#include "ass6_grp61_translator.h"
#include <iostream>
#include <cstring>
#include <string>
...
// asm file stream								
vector <quad> Array;						
string asmfilename="ass6_grp61_";		
string inputfile="ass6_grp61_test";		
...
void computeActivationRecord(symtable* st) {
	...
	for (list <sym>::iterator it = st->table.begin(); it!=st->table.end(); it++) {
		...
		if (it->category =="param") {
			// assign it to be param in activation record
			st->ar [it->name] = param;
			// add the size of the entry				
			param +=it->size;					
		}
